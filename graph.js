...
let emptyData = [
{'sensorType': 'sound', 'value': '15770.6', 'timeStamp': 1531192827266},
...
/**
 * Updates the already drawn graph.
 */
...
/**
 * Draws the graph from scratch, then updates it on a schedule.
 */
function drawGraph() {
    /* show the graph with dummy data so that
       the update has something to update.*/
    Plotly.plot('graph', getData(emptyData), graphLayout);

    let startTime = new Date(Date.now() + 3000);

    // let j =
    // need j later if you ever want to cancel the schedule
    schedule.scheduleJob({start: startTime,
                                  rule: '*/1 * * * * *'},
                                 function() {
